...
printTests =
  testGroup
    ...
    [ testCase "Print (int)" $
      ...
      --
      testCase "Print (bool)" $
        eval' (Print "flag" (CstBool True))
          @?= (["flag: True"], Right (ValBool True)),
      --
      testCase "Print (function)" $
        fst (eval' (Print "f" (Lambda "x" (Var "x"))))
          @?= ["f: #<fun>"],
      --
      testCase "Print (sequenced via Let)" $
        eval'
          ( Let
              "x"
              (Print "foo" (CstInt 2))
      ...
      --
      testCase "Print (output kept after later failure)" $
        eval'
          ( Let
              "x"
              (Print "foo" (CstInt 2))
      ...
      --
      testCase "Print (output kept across TryCatch)" $
        eval'
          ( TryCatch
              (Add (Print "foo" (CstInt 1)) (Div (CstInt 1) (CstInt 0)))
              (Print "bar" (CstInt 3))
          )
          @?= (["foo: 1", "bar: 3"], Right (ValInt 3))
    ]
